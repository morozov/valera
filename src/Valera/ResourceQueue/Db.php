...
QUERY;
        ...
        $stmt->bindValue(':resource_hash', $resource->getHash(), PDO::PARAM_INT);
        $stmt->execute();
    }
...
QUERY;
        ...
        $stmt = $this->conn->prepare($query);
        $stmt->bindValue(':hash', $resource->getHash(), PDO::PARAM_INT);
        $stmt->bindValue(':data', serialize($resource), PDO::PARAM_STR);
        $stmt->execute();
    }
